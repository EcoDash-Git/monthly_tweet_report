...
```{r}
# set English weekday names for the whole session
Sys.setlocale("LC_TIME", "C")         # or "en_US.UTF-8" on Linux/macOS
...
df_month <- df %>%
  filter(date >= month_start,
         date <  ceiling_date(month_start, "month")) %>%
  mutate(
    month_day  = wday(date, label = TRUE, abbr = FALSE, locale = "en_US"),
    month_hour = hour(date)
  )
